Goal #
The goal is to write a program that counts the occurrences of individual characters in a given file. When writing the program, you can assume that each character is represented by a single byte.

To speed up computations, the program should use shared memory for synchronization between multiple processes. For easier file processing, the file should be mapped into the process memory using the mmap function.

Stages #
Open the file using the mmap function in the parent process.
Print its contents to the standard output.
The use of streams and the read function is prohibited.

Implement the logic for counting character occurrences in the file.
At the end of the program execution, print a summary of how many times each character appeared in the file.

Distribute the workload across N child processes.
...
Add handling for the case where a child process terminates unexpectedly.
